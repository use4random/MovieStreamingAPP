...
async function untracedInstallBuilders(buildersDir, buildersToAdd) {
  ...
  try {
    const { stderr } = await (0, import_execa.default)("npm", installArgs, {
      cwd: buildersDir,
      stdio: "pipe",
      reject: true
    });
...
async function resolveBuilders(buildersDir, builderSpecs, resolvedSpecs, allowedPinDriftSpecs) {
  ...
  for (const spec of builderSpecs) {
    ...
    try {
      ...
      if (isBareSpec(parsed) && peerVersion && (0, import_semver2.valid)(peerVersion) && builderPkg.version !== peerVersion && !allowedPinDriftSpecs?.has(spec)) {
      ...
      if (isBareSpec(parsed) && peerVersion && isRemoteBuilderPin(peerVersion) && pkgPath.startsWith(buildersDir)) {
        let installedSpec;
        try {
          const buildersPkg = await (0, import_fs_extra2.readJSON)(join2(buildersDir, "package.json"));
